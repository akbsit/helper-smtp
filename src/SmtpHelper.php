<?php namespace Akbsit\HelperSmtp;

use Exception;

class SmtpHelper
{
    public array $arError = ['status' => false];
    private array $arSettings = [];

    public function __construct(array $arSettings)
    {
        try {
            if (!empty($arSettings['maillogin'])
            ...
            if (empty($arSettings['port'])) {
                $this->arSettings['port'] = 25;
            }

            if (empty($arSettings['charset'])) {
                $this->arSettings['charset'] = 'utf-8';
            }

            if (empty($arSettings['rpvalid']['mail'])) {
    ...
    public function send(string $sMail, string $sSubject = '', string $sMessage = ''):
    bool
    {
    ...
    private function getData($rConnection, string $sResponse): bool
    {
